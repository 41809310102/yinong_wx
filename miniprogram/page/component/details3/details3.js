    ...
    setTimeout(function() {
      ...
      setTimeout(function() {
        ...
        if (app.globalData.register == false) {
          ...
          db.collection("shopping_shouchan").where({ //判断购物车里面的商品是否已经被添加，如果已添加则修改里面的total值即可
            url: current_goods.url,
            ...
            success(res) {
              ...
              if (res.data.length > 0) {
                ...
                db.collection("shopping_shouchan").doc(res.data[0]._id).update({
                  data: {
                    total: num + total
                  },
                  success(res) {
                ...
                db.collection("shopping_shouchan").add({
                  data: {
                    'url': current_goods.url,
